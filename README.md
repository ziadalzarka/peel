...
- **A merge conflict is a file like any other.** Mid-merge, git has no single
  ...
  additions you can scroll, comment on and expand around. Conflicted files come
  first in the diff, and the file tree splits in two while one is open,
  `conflicts` at the top and `the rest` below, so what is waiting on a decision
  is never buried. `s` is still how it is marked
