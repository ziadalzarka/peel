...
internal/tui/model.go:412
  user (resolved): this leaks the tx
...
own, and then copied whole: the agent's notes in it are the other side of that
...
open note of the reviewer's is left out, on screen or not: one only the agent
...
name, where `C` marks them as the agent's in the review it hands over and `X` clears them
