...
| **A Markdown file is drawn to be read** | an added line in a `.md`, `.markdown`, `.mkd` or `.mdown` file gets a green `│` where the `+` goes and no green background; a removed line keeps its red. Headings and bold text are bold, italics italic, link text underlined with the address dimmed, table pipes dimmed | Added 2026-09-15. A change to a doc is mostly whole paragraphs of new text, and a green background under every line of it made the words themselves hard to read. The bar still says the line is new, in the column the `+` used to, without colouring the text. Removed lines keep the red because a reader has to notice what was taken out. The styling comes from a small lexer of peel's own rather than chroma's Markdown one, which needs a newline at the end of the line and a space before emphasis, and a diff line gives it neither. It works one line at a time like the rest of the highlighting, so it styles only what fits on one line: a heading, emphasis, a link, a table row. Outside a drawn table it only adds colour and never adds or removes a character, since the diff has to show the file as it is |
